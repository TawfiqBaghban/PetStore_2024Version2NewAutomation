...
public class UserTests {		//	api.test package --> UsertTests class.
...
//		just by mentioning User_EndPoints class and it's method createUser() method and passing (userPayload)
//		we got the response and stored it in a variable (response). now we can many validation on the response.
		
		Response response = User_EndPoints.createUser(userPayload); 	//	passing whole data here
...
//		parameter to updateUser() method.
		...
		Response response = User_EndPoints.updateUser(this.userPayload.getUsername(),userPayload); // updated data
